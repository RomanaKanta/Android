...
public class SeekTest {
    ...
    static boolean CanSeekAccurately(SharedPreferences prefs) {
        Log.i("SM_Ringtoner", "Running CanSeekAccurately");
        ...
        if (now - testDate < oneWeekMS) {
            Log.i("SM_Ringtoner", "Fast MP3 seek result cached: " + result);
            return result;
        }
        ...
        if (!ok) {
            Log.i("SM_Ringtoner", "Couldn't find temporary filename");
            return false;
        }

        Log.i("SM_Ringtoner", "Writing " + filename);

        try {
            file.createNewFile();
        } catch (Exception e) {
            // Darn, couldn't output for writing
            Log.i("SM_Ringtoner", "Couldn't output for writing");
            return false;
        }
        ...
        try {
            ...
            Log.i("SM_Ringtoner", "Couldn't write temp silence file");
            try {
                file.delete();
            } catch (Exception e2) {}
            return false;
        ...
        try {
            Log.i("SM_Ringtoner", "File written, starting to play");
            ...
            player.setDataSource(subsetInputStream.getFD(),
                                 start,
                                 len);
            Log.i("SM_Ringtoner", "Preparing");
            player.prepare();
            before = 0;
            after = 0;
            player.setOnCompletionListener(new OnCompletionListener() {
                    public synchronized void onCompletion(MediaPlayer arg0) {
                        Log.i("SM_Ringtoner", "Got callback");
                        after = System.currentTimeMillis();
                    }
                });

            Log.i("SM_Ringtoner", "Starting");
            player.start();

            for (int i = 0; i < 200 && before == 0; i++) {
                if (player.getCurrentPosition() > 0) {
                    Log.i("SM_Ringtoner", "Started playing after " + (i * 10) +
                          " ms");
            ...
            if (before == 0) {
                Log.i("SM_Ringtoner", "Never started playing.");
                Log.i("SM_Ringtoner", "Fast MP3 seek disabled by default");
                try {
                    file.delete();
                } catch (Exception e2) {}
            ...
            Log.i("SM_Ringtoner", "Sleeping");
            for (int i = 0; i < 300 && after == 0; i++) {
                Log.i("Ringdroid", "Pos: " + player.getCurrentPosition());
                Thread.sleep(10);
            }

            Log.i("SM_Ringtoner", "Result: " + before + ", " + after);

            if (after > before && after < before + 2000) {
                long delta = after > before? after - before: -1;
                Log.i("SM_Ringtoner", "Fast MP3 seek enabled: " + delta);
                result = true;
            } else {
                Log.i("SM_Ringtoner", "Fast MP3 seek disabled");
            }
        } catch (Exception e) {
            e.printStackTrace();
            Log.i("SM_Ringtoner", "Couldn't play: " + e.toString());
            Log.i("SM_Ringtoner", "Fast MP3 seek disabled by default");
            try {
                file.delete();
            } catch (Exception e2) {}
