...
public class CropImageView extends FrameLayout {
	...
	/**
	 * Progress bar widget to show progress bar on async image loading and
	 * cropping.
	 */
	...
	public CropImageView(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			try {
				guidelines = ta.getInteger(
						R.styleable.CropImageView_guidelines, guidelines);
				fixAspectRatio = ta.getBoolean(
						R.styleable.CropImageView_fixAspectRatio,
						Defaults.DEFAULT_FIXED_ASPECT_RATIO);
				aspectRatioX = ta.getInteger(
						R.styleable.CropImageView_aspectRatioX,
						Defaults.DEFAULT_ASPECT_RATIO_X);
				aspectRatioY = ta.getInteger(
						R.styleable.CropImageView_aspectRatioY,
						Defaults.DEFAULT_ASPECT_RATIO_Y);
				scaleType = Defaults.VALID_SCALE_TYPES[ta.getInt(
						R.styleable.CropImageView_scaleType,
						Defaults.DEFAULT_SCALE_TYPE_INDEX)];
				cropShape = Defaults.VALID_CROP_SHAPES[ta.getInt(
						R.styleable.CropImageView_cropShape,
						Defaults.DEFAULT_CROP_SHAPE_INDEX)];
				snapRadius = ta.getFloat(R.styleable.CropImageView_snapRadius,
						snapRadius);
				borderLineThickness = ta.getFloat(
						R.styleable.CropImageView_borderLineThickness,
						borderLineThickness);
				borderLineColor = ta.getInteger(
						R.styleable.CropImageView_borderLineColor,
						borderLineColor);
				borderCornerThickness = ta.getFloat(
						R.styleable.CropImageView_borderCornerThickness,
						borderCornerThickness);
				borderCornerOffset = ta.getFloat(
						R.styleable.CropImageView_borderCornerOffset,
						borderCornerOffset);
				borderCornerLength = ta.getFloat(
						R.styleable.CropImageView_borderCornerLength,
						borderCornerLength);
				borderCornerColor = ta.getInteger(
						R.styleable.CropImageView_borderCornerColor,
						borderCornerColor);
				guidelinesThickness = ta.getFloat(
						R.styleable.CropImageView_guidelinesThickness,
						guidelinesThickness);
				guidelinesColor = ta.getInteger(
						R.styleable.CropImageView_guidelinesColor,
						guidelinesColor);
				backgroundColor = ta.getInteger(
						R.styleable.CropImageView_backgroundColor,
						backgroundColor);
				mShowProgressBar = ta.getBoolean(
						R.styleable.CropImageView_showProgressBar,
						mShowProgressBar);
			} finally {
				ta.recycle();
			}
		...
		mCropOverlayView.setInitialAttributeValues(cropShape, snapRadius,
				guidelines, fixAspectRatio, aspectRatioX, aspectRatioY,
				borderLineThickness, borderLineColor, borderCornerThickness,
				borderCornerOffset, borderCornerLength, borderCornerColor,
				guidelinesThickness, guidelinesColor, backgroundColor);
...
//		if (cropShape == CropShape.HEXAGON) {
//			mCropOverlayView.setCropShape(CropShape.RECTANGLE);
//
////			mCropOverlayView.setHexaFrame(true);
//		} else {
			mCropOverlayView.setCropShape(cropShape);

//			mCropOverlayView.setHexaFrame(false);
//		}
	}
	...
	public Rect getActualCropRect() {
		if (mBitmap != null) {
			final Rect displayedImageRect = BitmapUtil.getBitmapRect(mBitmap,
					mImageView, mImageView.getScaleType());

			// Get the scale factor between the actual Bitmap dimensions and the
			// displayed dimensions for width.
			final float actualImageWidth = mBitmap.getWidth();
			final float displayedImageWidth = displayedImageRect.width();
			final float scaleFactorWidth = actualImageWidth
					/ displayedImageWidth;
			...
			// Correct for floating point errors. Crop rect boundaries should
			// not exceed the source Bitmap bounds.
			...
			return new Rect((int) actualCropLeft, (int) actualCropTop,
					(int) actualCropRight, (int) actualCropBottom);
		} else {
			return null;
		}
	}
	...
	/**
	 * Gets the crop window's position relative to the source Bitmap (not the
	 * image displayed in the CropImageView) and the original rotation.
	 * 
	 * @return a RectF instance containing cropped area boundaries of the source
	 *         Bitmap
	 */
	...
	public Rect getActualCropRectNoRotation() {
		if (mBitmap != null) {
			...
			if (rotateSide == 1) {
				rect.set(rect.top, mBitmap.getWidth() - rect.right,
						rect.bottom, mBitmap.getWidth() - rect.left);
			} else if (rotateSide == 2) {
				rect.set(mBitmap.getWidth() - rect.right, mBitmap.getHeight()
						- rect.bottom, mBitmap.getWidth() - rect.left,
						mBitmap.getHeight() - rect.top);
			} else if (rotateSide == 3) {
				rect.set(mBitmap.getHeight() - rect.bottom, rect.left,
						mBitmap.getHeight() - rect.top, rect.right);
			}
			rect.set(rect.left * mLoadedSampleSize, rect.top
					* mLoadedSampleSize, rect.right * mLoadedSampleSize,
					rect.bottom * mLoadedSampleSize);
			return rect;
		} else {
			return null;
		}
	 ...
	 * If image loaded from URI will use sample size to fit in the requested
	 * width and height down-sampling if required - optimization to get best
	 * size to quality.
	 * 
	 * @return a new Bitmap representing the cropped image
	 */
	public Bitmap getCroppedImage(int reqWidth, int reqHeight) {
		if (mBitmap != null) {
			if (mLoadedImageUri != null && mLoadedSampleSize > 1) {
				return BitmapUtil.cropBitmap(getContext(), mLoadedImageUri,
						getActualCropRectNoRotation(), mDegreesRotated,
						reqWidth, reqHeight);
			} else {
	 ...
	 * Same as {@link #getCroppedImage()} (as the bitmap is rectangular by
	 * nature) but the pixels beyond the oval crop will be transparent.
	 * 
	 ...
	 * {@link #setOnGetCroppedImageCompleteListener(CropImageView.OnGetCroppedImageCompleteListener)}
	 * .
	 */
	 ...
	 */
	public void getCroppedImageAsync(CropShape cropShape, int reqWidth,
			int reqHeight) {
		if (mOnGetCroppedImageCompleteListener == null) {
			throw new IllegalArgumentException(
					"OnGetCroppedImageCompleteListener is not set");
		}
		BitmapCroppingWorkerTask currentTask = mBitmapCroppingWorkerTask != null ? mBitmapCroppingWorkerTask
				.get() : null;
		if (currentTask != null) {
		...
		mBitmapCroppingWorkerTask = mLoadedImageUri != null
				&& mLoadedSampleSize > 1 ? new WeakReference<>(
				new BitmapCroppingWorkerTask(this, mLoadedImageUri,
						getActualCropRectNoRotation(), cropShape,
						mDegreesRotated, reqWidth, reqHeight))
				: new WeakReference<>(new BitmapCroppingWorkerTask(this,
						mBitmap, getActualCropRect(), cropShape));
	 ...
	 * {@link #setImageUriAsync(android.net.Uri)}) is complete (successful or failed).
	 */
	public void setOnSetImageUriCompleteListener(
			OnSetImageUriCompleteListener listener) {
		mOnSetImageUriCompleteListener = listener != null ? new WeakReference<>(
				listener) : null;
	}

	/**
	 * Set the callback to be invoked when image async cropping (
	 * {@link #getCroppedImageAsync()}) is complete (successful or failed).
	 */
	public void setOnGetCroppedImageCompleteListener(
			OnGetCroppedImageCompleteListener listener) {
		mOnGetCroppedImageCompleteListener = listener != null ? new WeakReference<>(
				listener) : null;
	}
	...
	public void setImageBitmap(Bitmap bitmap, ExifInterface exif) {
		if (bitmap != null && exif != null) {
			BitmapUtil.RotateBitmapResult result = BitmapUtil
					.rotateBitmapByExif(bitmap, exif);
	...
	/**
	 * Sets a bitmap loaded from the given Android URI as the content of the
	 * CropImageView.<br>
	 ...
	 * @deprecated Use {@link #setImageUriAsync(android.net.Uri)} for better async handling
	 */
	@Deprecated
	public void setImageUri(Uri uri) {
		if (uri != null) {
			...
			BitmapUtil.DecodeBitmapResult decodeResult = BitmapUtil
					.decodeSampledBitmap(getContext(), uri, width, height);

			BitmapUtil.RotateBitmapResult rotateResult = BitmapUtil
					.rotateBitmapByExif(getContext(), decodeResult.bitmap, uri);
	...
	private void setImageUriAsync(Uri uri, Integer preSetRotation) {
		if (uri != null) {
			BitmapLoadingWorkerTask currentTask = mBitmapLoadingWorkerTask != null ? mBitmapLoadingWorkerTask
					.get() : null;
			if (currentTask != null) {
				// cancel previous loading (no check if the same URI because
				// camera URI can be the same for different images)
				currentTask.cancel(true);
			}

			// either no existing task is working or we canceled it, need to
			// load new URI
			clearImage(true);
			mBitmapLoadingWorkerTask = new WeakReference<>(
					new BitmapLoadingWorkerTask(this, uri, preSetRotation));
	...
	/**
	 * On complete of the async bitmap loading by {@link #setImageUriAsync(android.net.Uri)}
	 * set the result to the widget if still relevant and call listener if set.
	 * 
	 * @param result
	 *            the result of bitmap loading
	 */
	void onSetImageUriAsyncComplete(BitmapLoadingWorkerTask.Result result) {
		...
		OnSetImageUriCompleteListener listener = mOnSetImageUriCompleteListener != null ? mOnSetImageUriCompleteListener
				.get() : null;
		if (listener != null) {
	...
	/**
	 * On complete of the async bitmap cropping by
	 * {@link #getCroppedImageAsync()} call listener if set.
	 * 
	 * @param result
	 *            the result of bitmap cropping
	 */
	void onGetImageCroppingAsyncComplete(BitmapCroppingWorkerTask.Result result) {
		...
		OnGetCroppedImageCompleteListener listener = mOnGetCroppedImageCompleteListener != null ? mOnGetCroppedImageCompleteListener
				.get() : null;
		if (listener != null) {
			listener.onGetCroppedImageComplete(this, result.bitmap,
					result.error);
		}
	}
	 ...
	 * Optionally clear full if the bitmap is new, or partial clear if the
	 * bitmap has been manipulated.
	 */
	 ...
	 * Full clear will also clear the data of the set image like Uri or Resource
	 * id while partial clear will only clear the bitmap and recycle if
	 * required.
	 */
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (mBitmap != null) {
			Rect bitmapRect = BitmapUtil.getBitmapRect(mBitmap, this,
	...
	/**
	 * Set visibility of progress bar when async loading/cropping is in process
	 * and show is enabled.
	 */
	...
	public enum CropShape {
        HEXAGON, RECTANGLE, OVAL,OCTAGON,PENTAGON,DIAMOND
	}
	...
	public interface OnSetImageUriCompleteListener {
		...
		/**
		 * Called when a crop image view has completed loading image for
		 * cropping.<br>
		 * If loading failed error parameter will contain the error.
		 * 
		 * @param view
		 *            The crop image view that loading of image was complete.
		 * @param uri
		 *            the URI of the image that was loading
		 * @param error
		 *            if error occurred during loading will contain the error,
		 *            otherwise null.
		 */
	...
	public interface OnGetCroppedImageCompleteListener {
		 ...
		 * 
		 * @param view
		 *            The crop image view that cropping of image was complete.
		 * @param bitmap
		 *            the cropped image bitmap (null if failed)
		 * @param error
		 *            if error occurred during cropping will contain the error,
		 *            otherwise null.
		 */
		void onGetCroppedImageComplete(CropImageView view, Bitmap bitmap,
                                       Exception error);
	}
	// endregion
}
