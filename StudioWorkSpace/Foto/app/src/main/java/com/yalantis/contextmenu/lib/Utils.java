...
public class Utils {
	...
	public static TextView getItemTextView(Context context,
			MenuObject menuItem, int menuItemSize) {
		TextView itemTextView = new TextView(context);
		RelativeLayout.LayoutParams textLayoutParams = new RelativeLayout.LayoutParams(
				ViewGroup.LayoutParams.WRAP_CONTENT, menuItemSize);
		itemTextView.setLayoutParams(textLayoutParams);
		// itemTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX,
		// context.getResources().getDimension(R.dimen.menu_text_size));
		itemTextView.setText(menuItem.getTitle());
		
		itemTextView.setPadding(0, 0, (int) context.getResources()
				.getDimension(R.dimen.text_right_padding), 0);
		itemTextView.setGravity(Gravity.CENTER_VERTICAL);
		int textColor = menuItem.getTextColor() == 0 ? context.getResources()
				.getColor(android.R.color.white) : menuItem.getTextColor();
		itemTextView.setTextColor(textColor);
		itemTextView.setTextAppearance(
				context,
				menuItem.getMenuTextAppearanceStyle() > 0 ? menuItem
						.getMenuTextAppearanceStyle()
						: R.style.TextView_DefaultStyle);
		
		itemTextView.setTypeface(Typeface.createFromAsset(context
	...
	public static View getDivider(Context context, MenuObject menuItem) {
		...
		int dividerColor = menuItem.getDividerColor() == Integer.MAX_VALUE ? context
				.getResources().getColor(R.color.divider_color) : menuItem
				.getDividerColor();
	...
	@SuppressLint("NewApi")
	public static RelativeLayout getImageWrapper(Context context,
			MenuObject menuItem, int menuItemSize,
...
//		}
		...
		if (menuItem.getBgColor() != 0) {
			...
			imageWrapper.setBackgroundColor(context.getResources().getColor(
