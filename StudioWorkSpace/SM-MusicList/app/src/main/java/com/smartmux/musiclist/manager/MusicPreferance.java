package com.smartmux.musiclist.manager;
...
public class MusicPreferance {
    ...
    public static void setShowMusicDetail(Context context, boolean show) {
        SharedPreferences.Editor editor = getPreferanse(context).edit();
        editor.putBoolean("showDetail", show);
        editor.commit();
    }

    public static boolean showMusicDetail(Context context) {
        SharedPreferences mSharedPreferences = getPreferanse(context);
        return mSharedPreferences.getBoolean("showDetail", true);
    }
    ...
    public static ArrayList<SongDetail> getPlaylist(Context context) {
        if (playlist == null || playlist.isEmpty()) {
            ...
            playlist = PhoneMediaControl.getInstance().getList(context, id, PhoneMediaControl.SonLoadFor.values()[type], path,0);
        }
        return playlist;
    }
    ...
    public static ArrayList<SongDetail> getPlaylist(Context context, String path) {
        ...
        MediaController.getInstance().path = path;
        playlist = PhoneMediaControl.getInstance().getList(context, -1, PhoneMediaControl.SonLoadFor.Musicintent, path,0);
        if (playlist != null && !playlist.isEmpty())
            playingSongDetail = playlist.get(0);
        return playlist;
    }

    public static void shufflPlaylist(Context context) {
        ...
        if (playlist == null || playlist.isEmpty()) {
            ...
            MediaController.getInstance().currentPlaylistNum = getLastPosition(context);
            MediaController.getInstance().path = path;
            shuffledPlaylist = PhoneMediaControl.getInstance().getList(context, id, PhoneMediaControl.SonLoadFor.values()[type], path,0);
        }else{
