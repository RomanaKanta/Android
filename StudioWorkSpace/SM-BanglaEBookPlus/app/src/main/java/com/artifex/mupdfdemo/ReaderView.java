...
public class ReaderView extends AdapterView<Adapter> implements GestureDetector.OnGestureListener,
		ScaleGestureDetector.OnScaleGestureListener, GestureDetector.OnDoubleTapListener, Runnable {
	...
	public int pageWidth, pageHeight, pageTop, pageLeft, pageRight, pageBottom;
...
//		throw new UnsupportedOperationException(getContext().getString(R.string.not_supported));
	...
	public boolean onDoubleTap(MotionEvent e) {
		if (mScale != 1.0f) {
			...
			View v = mChildViews.get(mCurrent);
			if (v != null) {
				mXScroll = ((-pageLeft) + fitLeft);
				mYScroll = ((-pageTop) + fitTop);
			}
		} else {
			...
			if (v != null) {
				...
				float lpX = spX * mScale;
				float lpY = spY * mScale;

//				int Left = (int) ((MuPDFActivity.disPlayWidth / 2) - lpX);
//				int Top = (int) ((MuPDFActivity.disPlayHeight / 2) - lpY);

//				if ((MuPDFActivity.disPlayWidth / 2) > lpX) {
//					Left = -fitLeft;
//				} else if (e.getX() > (MuPDFActivity.disPlayWidth / 2)) {
//					Left = Left + fitLeft;
//				}
//				if ((MuPDFActivity.disPlayHeight / 2) > lpY) {
//					Top = -fitTop;
//				} else if (e.getY() > (MuPDFActivity.disPlayHeight / 2)) {
//					Top = Top + fitTop;
//				}
//
//				mXScroll = Left;
