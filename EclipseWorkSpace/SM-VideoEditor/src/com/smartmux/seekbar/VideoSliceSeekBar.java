...
public class VideoSliceSeekBar extends ImageView {
	...
	// params
	// private Bitmap thumbSliceLeft = BitmapFactory.decodeResource(
	// getResources(), R.drawable.seek_pressed);
	private Bitmap thumbSlice = BitmapFactory.decodeResource(getResources(),
			R.drawable.slice);
	private Bitmap thumbCurrentVideoPosition = BitmapFactory.decodeResource(
			getResources(), R.drawable.leftthumb);
	private int progressMinDiff = 15; // percentage
	private int progressColor = getResources().getColor(R.color.gray);
	private int secondaryProgressColor = Color.parseColor("#90000000");
	private int progressHalfHeight = 3;
	private int thumbPadding = getResources().getDimensionPixelOffset(
			R.dimen.default_margin);
	private int maxValue = 100;
	private RectF rect;
	private Rect sRect;
	private int progressMinDiffPixels;
	private int thumbSliceLeftX, thumbSliceRightX;
	private int thumbCurrentVideoPositionX = thumbPadding;
	...
	private int progressBottom = 100;
	...
	public void getBitmapList(Bitmap list) {
		if (bmp != null) {
			bmp.recycle();
		}
		bmp = list;
	...
	protected void onDraw(Canvas canvas) {
		...
		rect = new RectF(thumbPadding, progressTop, getWidth() - thumbPadding,
				progressBottom);
		...
		rect = new RectF(thumbPadding, progressTop, thumbSliceLeftX,
				progressBottom);
		canvas.drawBitmap(thumbSlice, thumbSliceLeftX - thumbSliceHalfWidth,
				thumbSliceY, null);
		canvas.drawRect(rect, paint);

		// //// RIGHT THUMB
		rect = new RectF(thumbSliceRightX, thumbSliceY, getWidth()
				- thumbPadding, progressBottom);
		canvas.drawBitmap(thumbSlice, thumbSliceRightX - thumbSliceHalfWidth,
				thumbSliceY, null);
		canvas.drawRect(rect, paint);
		...
		canvas.drawBitmap(thumbCurrentVideoPosition, thumbCurrentVideoPositionX
				- thumbCurrentVideoPositionHalfWidth,
				thumbCurrentVideoPositionY, null);
		// }
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (!blocked) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				} else if (mx - thumbSliceLeftX + thumbSliceHalfWidth < thumbSliceRightX
						- thumbSliceHalfWidth - mx) {
					selectedThumb = SELECT_THUMB_LEFT;
				} else if (mx - thumbSliceLeftX + thumbSliceHalfWidth > thumbSliceRightX
						- thumbSliceHalfWidth - mx) {
					selectedThumb = SELECT_THUMB_RIGHT;
			...
			case MotionEvent.ACTION_MOVE:
				if ((mx <= thumbSliceLeftX + thumbSliceHalfWidth
						+ progressMinDiffPixels && selectedThumb == SELECT_THUMB_RIGHT)
						|| (mx >= thumbSliceRightX - thumbSliceHalfWidth
