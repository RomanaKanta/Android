...
public class PBIabHelper {
	 ...
	 */
	public PBIabHelper(Context ctx, String base64PublicKey) {
	...
	public interface OnIabSetupFinishedListener {
		 ...
		 * @param data
		 *            Goods information available for purchase.
		 */
	 ...
	 * 
	 * <p>
	 * Some code added so now it can also brings back goods information
	 * available for purchase in callback listener.
	 * </p>
	 * 
	 * @param listener
			...
			public void onServiceConnected(ComponentName name, IBinder service) {
				...
				try {
					...
					if (response != BILLING_RESPONSE_RESULT_OK) {
						if (listener != null)
							listener.onIabSetupFinished(new PBIabResult(
									response,
					...
					if (listener != null) {
						listener.onIabSetupFinished(
								new PBIabResult(IABHELPER_REMOTE_EXCEPTION,
				...
				try {
					...
					skuList.add("maple1");
					skuList.add("maple5");
					skuList.add("maple8");  // Newly added for 800 Yen item
					skuList.add("maple10");
					skuList.add("maple15");
					skuList.add("maple30");
					skuList.add("maple50");
					skuList.add("maple80");
					skuList.add("maple100");
					//Test Item added
					/*skuList.add("testmanaged5");
					skuList.add("testunmanaged5");*/


					Bundle querySkus = new Bundle();
					querySkus.putStringArrayList(GET_SKU_DETAILS_ITEM_LIST,
							skuList);
	...
	public interface OnIabPurchaseFinishedListener {
		 ...
		 */
		public void onIabPurchaseFinished(PBIabResult result, PBPurchase info);
	}
	...
	public void launchPurchaseFlow(Activity act, String sku, int requestCode,
		...
		try {
			...
			if (listener != null) {
				PBIabResult result = new PBIabResult(ResponseHandle.CODE_400,
						"Playストアにアップデートする必要が有ります。");
		...
		try {
			...
			if (response != BILLING_RESPONSE_RESULT_OK) {
				...
				result = new PBIabResult(response, "Unable to buy item");
				if (listener != null)
					listener.onIabPurchaseFinished(result, null);
			}
			...
			result = new PBIabResult(IABHELPER_SEND_INTENT_FAILED,
			...
			result = new PBIabResult(IABHELPER_REMOTE_EXCEPTION,
			...
			result = new PBIabResult(IABHELPER_UNKNOWN_ERROR,
					"Unexpected exception while starting purchase flow");
			if (listener != null)
				listener.onIabPurchaseFinished(result, null);
		}
	}
	...
	public boolean handleActivityResult(int requestCode, int resultCode,
		...
		if (data == null) {
			...
			result = new PBIabResult(IABHELPER_BAD_RESPONSE,
		...
		if (resultCode == Activity.RESULT_OK
			...
			PBPurchase purchase = null;
			try {
				purchase = new PBPurchase(purchaseData, dataSignature);
				String sku = purchase.getSku();

				// Verify signature
				if (!PBSecurity.verifyPurchase(mSignatureBase64, purchaseData,
						dataSignature)) {
					logError("Purchase signature verification FAILED for sku "
							+ sku);
					result = new PBIabResult(IABHELPER_VERIFICATION_FAILED,
			...
			if (mPurchaseListener != null) {
				mPurchaseListener.onIabPurchaseFinished(new PBIabResult(
			...
			if (mPurchaseListener != null) {
				result = new PBIabResult(responseCode,
			...
			logDebug("Purchase canceled - Response: "
					+ getResponseDesc(responseCode));
			result = new PBIabResult(IABHELPER_USER_CANCELLED, "User canceled.");
			if (mPurchaseListener != null)
				mPurchaseListener.onIabPurchaseFinished(result, null);
		} else {
			logError("Purchase failed. Result code: "
					...
					+ getResponseDesc(responseCode));
			result = new PBIabResult(IABHELPER_UNKNOWN_PURCHASE_RESPONSE,
	 ...
	 * @throws PBIabException
	 *             if a problem occurs while refreshing the inventory.
	 */
	public PBInventory queryInventory(boolean querySkuDetails,
			List<String> moreSkus) throws PBIabException {
		checkSetupDone("queryInventory");
		try {
			PBInventory inv = new PBInventory();
			int r = queryPurchases(inv);
			if (r != BILLING_RESPONSE_RESULT_OK) {
				throw new PBIabException(r,
			...
			throw new PBIabException(IABHELPER_REMOTE_EXCEPTION,
					"Remote exception while refreshing inventory.", e);
		} catch (JSONException e) {
			throw new PBIabException(IABHELPER_BAD_RESPONSE,
	...
	public interface QueryInventoryFinishedListener {
		 ...
		 *            The inventory.
		 */
		public void onQueryInventoryFinished(PBIabResult result, PBInventory inv);
	}
	...
	public void queryInventoryAsync(final boolean querySkuDetails,
		...
		(new Thread(new Runnable() {
			public void run() {
				PBIabResult result = new PBIabResult(
						BILLING_RESPONSE_RESULT_OK,
						"Inventory refresh successful.");
				PBInventory inv = null;
				try {
					inv = queryInventory(querySkuDetails, moreSkus);
				} catch (PBIabException ex) {
					result = ex.getResult();
				}

				flagEndAsync();

				final PBIabResult result_f = result;
				final PBInventory inv_f = inv;
	 ...
	 */
	void consume(PBPurchase itemInfo) throws PBIabException {
		checkSetupDone("consume");
		if (itemInfo == null) {
			throw new PBIabException(IABHELPER_UNKNOWN_PURCHASE_RESPONSE,
					"Null purchase info recieved.");
		}
		try {
			...
			if (token == null || token.equals("")) {
				...
				throw new PBIabException(IABHELPER_MISSING_TOKEN,
			...
			if (response == BILLING_RESPONSE_RESULT_OK) {
				...
				logDebug("Error consuming consuming sku " + sku + ". "
						+ getResponseDesc(response));
				throw new PBIabException(response, "Error consuming sku " + sku);
			}
		} catch (RemoteException e) {
			throw new PBIabException(IABHELPER_REMOTE_EXCEPTION,
	...
	public interface OnConsumeMultiFinishedListener {
		 ...
		 *            to each sku.
		 */
		public void onConsumeMultiFinished(List<PBPurchase> purchases,
				List<PBIabResult> results);
	}
	 ...
	 *            finishes.
	 */
	public void consumeAsync(PBPurchase purchase,
			OnConsumeFinishedListener listener) {
		checkSetupDone("consume");
		List<PBPurchase> purchases = new ArrayList<PBPurchase>();
	 ...
	 *            finishes.
	 */
	public void consumeAsync(List<PBPurchase> purchases,
	...
	int queryPurchases(PBInventory inv) throws JSONException, RemoteException {
		// Query purchases
		...
		do {
			...
			for (int i = 0; i < purchaseDataList.size(); ++i) {
				...
				if (PBSecurity.verifyPurchase(mSignatureBase64, purchaseData,
					...
					PBPurchase purchase = new PBPurchase(purchaseData,
							signature);
	...
	int querySkuDetails(PBInventory inv, List<String> moreSkus)
		...
		for (String thisResponse : responseList) {
			PBSkuDetails d = new PBSkuDetails(thisResponse);
	...
	void consumeAsyncInternal(final List<PBPurchase> purchases,
		...
		(new Thread(new Runnable() {
			public void run() {
				final List<PBIabResult> results = new ArrayList<PBIabResult>();
				for (PBPurchase purchase : purchases) {
					try {
						consume(purchase);
						results.add(new PBIabResult(BILLING_RESPONSE_RESULT_OK,
